...
1. **Room shape generalization** (`packages/core`) — L/cross/circle/triangle/polygon rooms, randomized via a weighted, config-editable table, rooms as a union of sub-shapes. Touches `placeRooms`, `carve`, `extractWalls`, `verticalLinks` — all currently rect-assuming. Explicitly sequenced after all three increments above; Increment 3's toggled-but-not-applied cells are a natural preview of what this phase would let become real shapes.
